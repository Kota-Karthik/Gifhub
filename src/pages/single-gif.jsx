...
const GifPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchGif = async () => {
      ...
      const { data:gif } = await gf.gif(gifId[gifId.length - 1]);
      setGif(gif);
      const { data:gifs } = await gf.related(gifId[gifId.length - 1], { limit: 10});
      console.log("related: ",gifs);
      setRelatedGifs(gifs);
    };
  ...
  const EmbedGif = () => {
    // Assignment
    alert("Embed functionality not yet implemented.");
  };
